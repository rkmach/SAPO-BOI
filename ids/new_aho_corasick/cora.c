...
static void ahocora_print_trie (struct ahocora_trie *trie)
{
        ...
        for (int i = 0 ; i < trie->size ; i++){
                ...
                printf ("parent: %d\n", cur_node->parent);
                printf ("suffix: %d[%c]\n", cur_node->suffix, cur_node->suffix);
                ...
                for (int j = 0 ; j < NUM_ACCEPTABLE_SYMBOLS ; j ++){
                        if (cur_node->basic_links[j] != -1)
                                printf ("\tbasic_link[%d(%c)]: %d\n", j, j,
                                                cur_node->basic_links[j]);
                }
                printf ("----- NODE %d -----\n\n", i);
        }
}
static int ahocora_lookup (struct ahocora_trie *trie, char *pattern, int size)
{
...
static int ahocora_find_best_suffix_link (struct ahocora_trie *trie,
                struct ahocora_node *node, char *longest_pattern,
...
static void __ahocora_build_dict_suffix_links (struct ahocora_trie *trie, struct ahocora_node *node, char *longest_pattern, int size)
{
        int target_node;
        char *original_pattern = longest_pattern;
        int original_size = size;
        for (int i = 0 ; i < original_size - 1 ; i++){
                if (size == 0)
                        break;
                target_node = ahocora_lookup (trie, longest_pattern + 1, size - 1);
                if (target_node == -1){
                        longest_pattern++;
                        size--;
                }
        ...
        for (int i = 0 ; i < NUM_ACCEPTABLE_SYMBOLS ; i++){
                ...
                __ahocora_build_dict_suffix_links (trie, next_node, longest_pattern, size);
                size = original_size;
        }
}
...
{
        char *longest_pattern = malloc (MAX_FRAME_SIZE);
        memset (longest_pattern, 0 , MAX_FRAME_SIZE);

        for (int i = 0 ; i < trie->size ; i++)
                trie->array[i]->dict_suffix_link = -1;

        __ahocora_build_dict_suffix_links (trie, trie->array[0], longest_pattern, 0);

        free (longest_pattern);

}

void ahocora_insert_pattern (struct ahocora_trie *trie, char *pattern,
                int pattern_size, int rule_sid)
{
        struct ahocora_node * cur_node = trie->array[0];

        for (int i = 0 ; i < pattern_size ; i++) {
                int cur_byte = pattern[i];
...
static inline int ahocora_count_dict_hits (struct ahocora_trie *trie, struct ahocora_node *node, int hits)
{
...
int ahocora_search (struct ahocora_trie *trie, char *input, int size)
{
        ...
        for (int i = 0 ; i < size + 1 ; i++){
                ...
                if (node->dict_suffix_link != -1) {
                        num_found_patterns += ahocora_count_dict_hits (trie, trie->array[node->dict_suffix_link], 0);
                }
...
int main ()
{
        ...
        */
        ...
        printf ("ahocora_search(): %d\n", ahocora_search (trie, "fooxarazacaralhabba", 84-65));
        return 0;
}
